...
router.post('/teachers', adminAuthMiddleware, upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            try {
                ...
                const birthDateStr = String(row['Doğum Tarihi'] || row['Dogum Tarihi'] || '').trim();
                const points = parseFloat(row['Puan'] || row['Atama Puanı'] || 0);
                const branch = String(row['Branş'] || row['Brans'] || '').trim();
                const currentAssignment = String(row['Mevcut Atama'] || row['Atama'] || '').trim();
                ...
                if (birthDateStr.includes('.')) {
                    ...
                    errors.push(`Satır ${i + 2}: Doğum tarihi formatı hatalı (GG.AA.YYYY olmalı)`);
                    errorCount++;
                    continue;
                }
                ...
                await pool.execute(
                    ...
                    [tcId, firstName, lastName, birthDate, points, branch, currentAssignment, passwordHash]
                );
...
router.post('/positions', adminAuthMiddleware, upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ message: 'Dosya yüklenmedi' });
        }
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            try {
                ...
                const schoolName = String(row['Okul Adı'] || row['Okul'] || '').trim();
                const district = String(row['İlçe'] || row['Ilce'] || '').trim();
                const branch = String(row['Branş'] || row['Brans'] || '').trim();
                const quota = parseInt(row['Kontenjan'] || row['Kota'] || 1);
                const statusStr = String(row['Durum'] || 'Aktif').trim().toLowerCase();
                const status = (statusStr === 'aktif' || statusStr === 'active') ? 'active' : 'inactive';
